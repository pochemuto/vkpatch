 ...
 */
function ready(source, url) 
{
	log('ready : ' + url);
	injectBuffer.push({source: source, url: url});
	waitPage.call();
};
...
function inject(values) 
{
	...
	while (pageInfo = injectBuffer.shift()) 
	{
		...
		for (var i=0; i<scriptsData.length; i++) 
		{
			...
			createScript(scriptData.content, scriptData.url, pageInfo.source, pageInfo.url);
		}
	}
